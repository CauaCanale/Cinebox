    ...
    <main class="container">
        <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-12 my-4">
                <img src="./assets/img/<?= ('poster/' . $value['poster']) ?>" alt="poster Jedi Survivor" class="foto ">
            </div>
            <div class="col-lg-6 col-md-6 col-sm-12 detalhe-produto py-5">
                <h1 class="titulo-jogo"><?= $value['nome'] ?></h1>

                <div class="genero">
                    <?php foreach ($valueGeneros as $gen) { ?>
                        <span style="background-color: #<?= $gen['cor'] ?>;"><?= $gen['nome'] ?></span>
                    <?php } ?>
                </div>

                <h3 class="preco">R$ <?= number_format($value['valor_ingresso'], 2, ',', '.') ?></h3>
